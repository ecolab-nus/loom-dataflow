...
module {
  func.func @flashattn_fwd(%arg0: memref<*xf16> {tt.divisibility = 16 : i32}, %arg1: memref<*xf16> {tt.divisibility = 16 : i32}, %arg2: memref<*xf16> {tt.divisibility = 16 : i32}, %arg3: memref<*xf16> {tt.divisibility = 16 : i32}, %arg4: index, %arg5: index, %arg6: index, %arg7: index, %arg8: index, %arg9: index) {
    ...
    %8:5 = scf.for %arg10 = %c0 to %c512 step %c32 iter_args(%arg11 = %c0_3, %arg12 = %c0_4, %arg13 = %5, %arg14 = %1, %arg15 = %4) -> (index, index, tensor<32xf32>, tensor<32xf32>, tensor<32x32xf32>) {
      %14 = affine.apply #map1()[%arg12]
      ...
      %17 = linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%16, %3 : tensor<32x32xf32>, tensor<32x32xf32>) outs(%16 : tensor<32x32xf32>) {
      ...
      %18 = linalg.generic {indexing_maps = [#map3, #map3, #map3], iterator_types = ["parallel"]} ins(%arg13, %reduced : tensor<32xf32>, tensor<32xf32>) outs(%arg13 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.maxnumf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %expanded_8 = tensor.expand_shape %18 [[0, 1]] output_shape [32, 1] : tensor<32xf32> into tensor<32x1xf32>
      %19 = linalg.generic {indexing_maps = [#map4, #map2], iterator_types = ["parallel", "parallel"]} ins(%expanded_8 : tensor<32x1xf32>) outs(%2 : tensor<32x32xf32>) attrs =  {broadcastDims = array<i64: 1>} {
      ^bb0(%in: f32, %out: f32):
        linalg.yield %in : f32
      } -> tensor<32x32xf32>
      %20 = linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%17, %19 : tensor<32x32xf32>, tensor<32x32xf32>) outs(%17 : tensor<32x32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.subf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32x32xf32>
      %21 = linalg.generic {indexing_maps = [#map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%20 : tensor<32x32xf32>) outs(%20 : tensor<32x32xf32>) {
      ^bb0(%in: f32, %out: f32):
        %37 = math.exp %in : f32
        linalg.yield %37 : f32
      } -> tensor<32x32xf32>
      ...
      %23 = linalg.generic {indexing_maps = [#map3, #map3, #map3], iterator_types = ["parallel"]} ins(%arg13, %18 : tensor<32xf32>, tensor<32xf32>) outs(%arg13 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.subf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %24 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel"]} ins(%23 : tensor<32xf32>) outs(%23 : tensor<32xf32>) {
      ^bb0(%in: f32, %out: f32):
        %37 = math.exp %in : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %25 = linalg.generic {indexing_maps = [#map3, #map3, #map3], iterator_types = ["parallel"]} ins(%arg14, %24 : tensor<32xf32>, tensor<32xf32>) outs(%arg14 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.mulf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %26 = linalg.generic {indexing_maps = [#map3, #map3, #map3], iterator_types = ["parallel"]} ins(%25, %reduced_10 : tensor<32xf32>, tensor<32xf32>) outs(%25 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.addf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %27 = affine.apply #map5()[%arg11]
      ...
      %30 = linalg.generic {indexing_maps = [#map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%21 : tensor<32x32xf32>) outs(%29 : tensor<32x32xf16>) {
      ^bb0(%in: f32, %out: f16):
        %37 = arith.truncf %in : f32 to f16
        linalg.yield %37 : f16
      } -> tensor<32x32xf16>
      %expanded_13 = tensor.expand_shape %24 [[0, 1]] output_shape [32, 1] : tensor<32xf32> into tensor<32x1xf32>
      %31 = linalg.generic {indexing_maps = [#map4, #map2], iterator_types = ["parallel", "parallel"]} ins(%expanded_13 : tensor<32x1xf32>) outs(%2 : tensor<32x32xf32>) attrs =  {broadcastDims = array<i64: 1>} {
      ^bb0(%in: f32, %out: f32):
        linalg.yield %in : f32
      } -> tensor<32x32xf32>
      %32 = linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%arg15, %31 : tensor<32x32xf32>, tensor<32x32xf32>) outs(%arg15 : tensor<32x32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.mulf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32x32xf32>
      %33 = linalg.matmul ins(%30, %28 : tensor<32x32xf16>, tensor<32x32xf16>) outs(%4 : tensor<32x32xf32>) -> tensor<32x32xf32>
      %34 = linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%32, %33 : tensor<32x32xf32>, tensor<32x32xf32>) outs(%32 : tensor<32x32xf32>) {
      ^bb0(%in: f32, %in_16: f32, %out: f32):
        %37 = arith.addf %in, %in_16 : f32
        linalg.yield %37 : f32
      } -> tensor<32x32xf32>
      %c32_14 = arith.constant 32 : index
      %35 = arith.addi %arg11, %c32_14 : index
      %c32_15 = arith.constant 32 : index
      %36 = arith.addi %arg12, %c32_15 : index
    ...
    %9 = linalg.generic {indexing_maps = [#map4, #map2], iterator_types = ["parallel", "parallel"]} ins(%expanded : tensor<32x1xf32>) outs(%2 : tensor<32x32xf32>) attrs =  {broadcastDims = array<i64: 1>} {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<32x32xf32>
    %10 = linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%8#4, %9 : tensor<32x32xf32>, tensor<32x32xf32>) outs(%8#4 : tensor<32x32xf32>) {
    ^bb0(%in: f32, %in_6: f32, %out: f32):
      %14 = arith.divf %in, %in_6 : f32
      linalg.yield %14 : f32
    } -> tensor<32x32xf32>
    %11 = tensor.empty() : tensor<32x32xf16>
    %12 = linalg.generic {indexing_maps = [#map2, #map2], iterator_types = ["parallel", "parallel"]} ins(%10 : tensor<32x32xf32>) outs(%11 : tensor<32x32xf16>) {
    ^bb0(%in: f32, %out: f16):
