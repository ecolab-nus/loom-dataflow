module {
  func.func @flashattn_fwd(%arg0: memref<*xf16> {tt.divisibility = 16 : i32}, %arg1: memref<*xf16> {tt.divisibility = 16 : i32}, %arg2: memref<*xf16> {tt.divisibility = 16 : i32}, %arg3: memref<*xf16> {tt.divisibility = 16 : i32}, %arg4: index, %arg5: index, %arg6: index, %arg7: index, %arg8: index, %arg9: index) {
    ...
    %8:5 = scf.for %arg10 = %c0 to %c512 step %c32 iter_args(%arg11 = %c0, %arg12 = %c0, %arg13 = %5, %arg14 = %1, %arg15 = %4) -> (index, index, tensor<32xf32>, tensor<32xf32>, tensor<32x32xf32>) {
      %14 = affine.apply affine_map<()[s0] -> (s0)>()[%arg12]
      %reinterpret_cast_4 = memref.reinterpret_cast %arg1 to offset: [%14], sizes: [32, 32], strides: [512, 1] : memref<*xf16> to memref<32x32xf16, strided<[512, 1], offset: ?>>
      ...
      %15 = bufferization.to_tensor %alloc_5 restrict writable : memref<32x32xf16> to tensor<32x32xf16>
      ...
      %18 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%arg13, %reduced : tensor<32xf32>, tensor<32xf32>) outs(%arg13 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_12: f32, %out: f32):
        %37 = arith.maxnumf %in, %in_12 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %expanded_6 = tensor.expand_shape %18 [[0, 1]] output_shape [32, 1] : tensor<32xf32> into tensor<32x1xf32>
      %19 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, 0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%expanded_6 : tensor<32x1xf32>) outs(%2 : tensor<32x32xf32>) attrs =  {broadcastDims = array<i64: 1>} {
      ^bb0(%in: f32, %out: f32):
        linalg.yield %in : f32
      } -> tensor<32x32xf32>
      ...
      %transposed_7 = linalg.transpose ins(%21 : tensor<32x32xf32>) outs(%2 : tensor<32x32xf32>) permutation = [1, 0] 
      %22 = linalg.fill ins(%cst_1 : f32) outs(%0 : tensor<32xf32>) -> tensor<32xf32>
      %reduced_8 = linalg.reduce ins(%transposed_7 : tensor<32x32xf32>) outs(%22 : tensor<32xf32>) dimensions = [0] 
        (%in: f32, %init: f32) {
          %37 = arith.addf %in, %init : f32
          linalg.yield %37 : f32
        }
      %23 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%arg13, %18 : tensor<32xf32>, tensor<32xf32>) outs(%arg13 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_12: f32, %out: f32):
        %37 = arith.subf %in, %in_12 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      ...
      %25 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%arg14, %24 : tensor<32xf32>, tensor<32xf32>) outs(%arg14 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_12: f32, %out: f32):
        %37 = arith.mulf %in, %in_12 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %26 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%25, %reduced_8 : tensor<32xf32>, tensor<32xf32>) outs(%25 : tensor<32xf32>) {
      ^bb0(%in: f32, %in_12: f32, %out: f32):
        %37 = arith.addf %in, %in_12 : f32
        linalg.yield %37 : f32
      } -> tensor<32xf32>
      %27 = affine.apply affine_map<()[s0] -> (s0 * 32)>()[%arg11]
      %reinterpret_cast_9 = memref.reinterpret_cast %arg2 to offset: [%27], sizes: [32, 32], strides: [32, 1] : memref<*xf16> to memref<32x32xf16, strided<[32, 1], offset: ?>>
      ...
      %28 = bufferization.to_tensor %alloc_10 restrict writable : memref<32x32xf16> to tensor<32x32xf16>
      ...
      %expanded_11 = tensor.expand_shape %24 [[0, 1]] output_shape [32, 1] : tensor<32xf32> into tensor<32x1xf32>
      %31 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, 0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%expanded_11 : tensor<32x1xf32>) outs(%2 : tensor<32x32xf32>) attrs =  {broadcastDims = array<i64: 1>} {
      ^bb0(%in: f32, %out: f32):
        linalg.yield %in : f32
      } -> tensor<32x32xf32>
      %32 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%arg15, %31 : tensor<32x32xf32>, tensor<32x32xf32>) outs(%arg15 : tensor<32x32xf32>) {
      ^bb0(%in: f32, %in_12: f32, %out: f32):
        %37 = arith.mulf %in, %in_12 : f32
        linalg.yield %37 : f32
      } -> tensor<32x32xf32>
      ...
      %35 = arith.addi %arg11, %c32 : index
      %36 = arith.addi %arg12, %c32 : index
    ...
    %reinterpret_cast_3 = memref.reinterpret_cast %arg3 to offset: [%13], sizes: [32, 32], strides: [32, 1] : memref<*xf16> to memref<32x32xf16, strided<[32, 1], offset: ?>>
    bufferization.materialize_in_destination %12 in writable %reinterpret_cast_3 : (tensor<32x32xf16>, memref<32x32xf16, strided<[32, 1], offset: ?>>) -> ()
    return
  }
}
