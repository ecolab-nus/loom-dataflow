  ...
  module {
    ...
    func.func @attention(%arg0: memref<2x4096x4096xf32>, %arg1: memref<2x4096x4096xf32>, %arg2: memref<2x4096x4096xf32>, %arg3: memref<2x4096x4096xf32>) {
      ...
      affine.parallel (%arg4, %arg5) = (0, 0) to (2 ceildiv symbol(%0), 4096 ceildiv symbol(%1)) {
        ...
        %subview = memref.subview %arg2[%8, %9, 0] [%0, %1, 4096] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> // subview of Q block with size [BB, BM, dim]
        %10 = bufferization.to_tensor %subview : memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> to tensor<?x?x4096xf32> // buffer Q to tensor
        %11:3 = affine.for %arg6 = 0 to #map()[%2] iter_args(%arg7 = %4, %arg8 = %5, %arg9 = %7) -> (tensor<?x?xf32>, tensor<?x?xf32>, tensor<?x?x4096xf32>) { // iter on N dimension
          %14 = arith.muli %arg6, %2 : index // iter_N * BN
          // NOTE: %arg0 is K^T
          %subview_5 = memref.subview %arg0[%8, 0, %14] [%0, 4096, %2] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x4096x?xf32, strided<[16777216, 4096, 1], offset: ?>> // subview of K^T block with size [BB, dim, BN]
          %15 = bufferization.to_tensor %subview_5 : memref<?x4096x?xf32, strided<[16777216, 4096, 1], offset: ?>> to tensor<?x4096x?xf32> // buffer K^T to tensor
          ...
          %20 = linalg.batch_matmul ins(%10, %15 : tensor<?x?x4096xf32>, tensor<?x4096x?xf32>) outs(%19 : tensor<?x?x?xf32>) -> tensor<?x?x?xf32> // Q @ K^T
          //** torch.amax(qk, -1) **/
          ...
          // acc *= alpha[:, :, None]
          %35 = linalg.generic {indexing_maps = [#map1, #map4, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%arg9, %expanded_9 : tensor<?x?x4096xf32>, tensor<?x?x1xf32>) outs(%6 : tensor<?x?x4096xf32>) {
          ^bb0(%in: f32, %in_11: f32, %out: f32):
            %41 = arith.mulf %in, %in_11 : f32
            linalg.yield %41 : f32
          } -> tensor<?x?x4096xf32>
          %subview_10 = memref.subview %arg1[%8, %14, 0] [%0, %2, 4096] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> // subview of V block with size [BB, BN, dim]
          %36 = bufferization.to_tensor %subview_10 : memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> to tensor<?x?x4096xf32>
          cf.assert %17, "mismatching contracting dimension"
          %37 = arith.index_cast %2 : index to i64
          %38 = arith.cmpi eq, %37, %37 : i64
          cf.assert %38, "mismatching contracting dimension"
          // p @ v
          %39 = linalg.batch_matmul ins(%28, %36 : tensor<?x?x?xf32>, tensor<?x?x4096xf32>) outs(%7 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
          // acc += p @ v
          %40 = linalg.generic {indexing_maps = [#map1, #map1, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%39, %35 : tensor<?x?x4096xf32>, tensor<?x?x4096xf32>) outs(%6 : tensor<?x?x4096xf32>) {
          ^bb0(%in: f32, %in_11: f32, %out: f32):
            %41 = arith.addf %in, %in_11 : f32
            linalg.yield %41 : f32
          } -> tensor<?x?x4096xf32>
          // yield m_ij as m_i, l_i, acc
          affine.yield %25, %34, %40 : tensor<?x?xf32>, tensor<?x?xf32>, tensor<?x?x4096xf32>
        }
        // l_i[:, :, None]
        ...
        // acc /= l_i[:, :, None]
        %12 = linalg.generic {indexing_maps = [#map1, #map4, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%11#2, %expanded : tensor<?x?x4096xf32>, tensor<?x?x1xf32>) outs(%6 : tensor<?x?x4096xf32>) {
        ^bb0(%in: f32, %in_5: f32, %out: f32):
          %14 = arith.divf %in, %in_5 : f32
          linalg.yield %14 : f32
        } -> tensor<?x?x4096xf32>
        %subview_4 = memref.subview %arg3[%8, %9, 0] [%0, %1, 4096] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> // subview of O block with size [BB, BM, dim]
