  ...
  module {
    ...
    func.func @attention(%arg0: memref<2x4096x4096xf32>, %arg1: memref<2x4096x4096xf32>, %arg2: memref<2x4096x4096xf32>, %arg3: memref<2x4096x4096xf32>) {
      ...
      affine.parallel (%arg4, %arg5) = (0, 0) to (2 ceildiv symbol(%0), 4096 ceildiv symbol(%1)) {
        ...
        %17 = loom.alloc [%0, %1, 4096] on @L1 : memref<?x?x4096xf32>
        %18 = loom.init_tensor %17[%0, %1, 4096] : memref<?x?x4096xf32> -> tensor<?x?x4096xf32>
        %19 = loom.alloc [%0, %1, 4096] on @L1 : memref<?x?x4096xf32>
        %20 = loom.init_tensor %19[%0, %1, 4096] : memref<?x?x4096xf32> -> tensor<?x?x4096xf32>
        %21 = loom.alloc [%0, %1, 4096] on @L1 : memref<?x?x4096xf32>
        %22 = loom.init_tensor %21[%0, %1, 4096] : memref<?x?x4096xf32> -> tensor<?x?x4096xf32>
        %23 = arith.muli %arg4, %0 : index
        %24 = arith.muli %arg5, %1 : index
        %25 = loom.subview %arg2[%23, %24, 0] [%0, %1, 4096] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>
        %26 = loom.copy_to_tensor %25, %17, interconnect : [], broadcast : [1, 1] : memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>, memref<?x?x4096xf32> -> tensor<?x?x4096xf32>
        %27 = linalg.fill ins(%cst_1 : f32) outs(%20 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
        ...
        %30:3 = affine.for %arg6 = 0 to affine_map<()[s0] -> (4096 ceildiv s0)>()[%2] iter_args(%arg7 = %29, %arg8 = %28, %arg9 = %27) -> (tensor<?x?xf32>, tensor<?x?xf32>, tensor<?x?x4096xf32>) {
          %33 = arith.muli %arg6, %2 : index
          %34 = loom.subview %arg0[%23, 0, %33] [%0, 4096, %2] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<2x4096x4096xf32> to memref<?x4096x?xf32, strided<[16777216, 4096, 1], offset: ?>>
          ...
          %37 = linalg.batch_matmul ins(%26, %35 : tensor<?x?x4096xf32>, tensor<?x4096x?xf32>) outs(%36 : tensor<?x?x?xf32>) -> tensor<?x?x?xf32>
          ...
          %46 = loom.subview %arg1[%23, %33, 0] [%0, %2, 4096] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>
          %47 = loom.copy_to_tensor %46, %3, interconnect : [], broadcast : [1, 1] : memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>, memref<?x?x4096xf32> -> tensor<?x?x4096xf32>
          %48 = linalg.fill ins(%cst_1 : f32) outs(%22 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
          %49 = linalg.batch_matmul ins(%41, %47 : tensor<?x?x?xf32>, tensor<?x?x4096xf32>) outs(%48 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
          %50 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%49, %arg9, %44 : tensor<?x?x4096xf32>, tensor<?x?x4096xf32>, tensor<?x?xf32>) outs(%arg9 : tensor<?x?x4096xf32>) {
          ^bb0(%in: f32, %in_4: f32, %in_5: f32, %out: f32):
          ...
          affine.yield %51, %45, %50 : tensor<?x?xf32>, tensor<?x?xf32>, tensor<?x?x4096xf32>
        }
        %31 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%30#2, %30#1 : tensor<?x?x4096xf32>, tensor<?x?xf32>) outs(%18 : tensor<?x?x4096xf32>) {
        ^bb0(%in: f32, %in_4: f32, %out: f32):
          %33 = arith.divf %in, %in_4 : f32
          linalg.yield %33 : f32
        } -> tensor<?x?x4096xf32>
        %32 = loom.subview %arg3[%23, %24, 0] [%0, %1, 4096] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>
