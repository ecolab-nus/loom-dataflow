module attributes {loom.block_size_0 = -1 : index, loom.block_size_1 = -1 : index, loom.block_size_3 = -1 : index} {
  func.func @attention(%arg0: memref<8x128x512xf16>, %arg1: memref<8x512x128xf16>, %arg2: memref<8x512x128xf16>, %arg3: memref<8x512x128xf16>) {
    ...
    affine.parallel (%arg4, %arg5) = (0, 0) to (8 ceildiv symbol(%0), 512 ceildiv symbol(%1)) {
      ...
      %37 = loom.subview %arg2[%35, %36, 0] [%0, %1, 128] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<8x512x128xf16> to memref<?x?x128xf16, strided<[65536, 128, 1], offset: ?>>
      %38 = loom.copy_to_tensor %37, %28, interconnect : [], broadcast : [1, 1] : memref<?x?x128xf16, strided<[65536, 128, 1], offset: ?>>, memref<?x?x128xf16> -> tensor<?x?x128xf16>
      %39 = linalg.fill ins(%cst_0 : f16) outs(%31 : tensor<?x?x128xf16>) -> tensor<?x?x128xf16>
      %40 = linalg.fill ins(%cst_1 : f16) outs(%12 : tensor<?x?xf16>) -> tensor<?x?xf16>
      %41 = linalg.fill ins(%cst_2 : f16) outs(%15 : tensor<?x?xf16>) -> tensor<?x?xf16>
      %42:3 = affine.for %arg6 = 0 to affine_map<()[s0] -> (512 ceildiv s0)>()[%2] iter_args(%arg7 = %41, %arg8 = %40, %arg9 = %39) -> (tensor<?x?xf16>, tensor<?x?xf16>, tensor<?x?x128xf16>) {
        %45 = arith.muli %arg6, %2 : index
        %46 = loom.subview %arg0[%35, 0, %45] [%0, 128, %2] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<8x128x512xf16> to memref<?x128x?xf16, strided<[65536, 512, 1], offset: ?>>
        %47 = loom.copy_to_tensor %46, %9, interconnect : [], broadcast : [1, 1] : memref<?x128x?xf16, strided<[65536, 512, 1], offset: ?>>, memref<?x128x?xf16> -> tensor<?x128x?xf16>
        %48 = linalg.fill ins(%cst_0 : f16) outs(%7 : tensor<?x?x?xf16>) -> tensor<?x?x?xf16>
        ...
        %50 = linalg.fill ins(%cst_2 : f16) outs(%18 : tensor<?x?xf16>) -> tensor<?x?xf16>
        ...
        %54 = linalg.fill ins(%cst_0 : f16) outs(%21 : tensor<?x?xf16>) -> tensor<?x?xf16>
        ...
        %58 = loom.subview %arg1[%35, %45, 0] [%0, %2, 128] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<8x512x128xf16> to memref<?x?x128xf16, strided<[65536, 128, 1], offset: ?>>
        %59 = loom.copy_to_tensor %58, %4, interconnect : [], broadcast : [1, 1] : memref<?x?x128xf16, strided<[65536, 128, 1], offset: ?>>, memref<?x?x128xf16> -> tensor<?x?x128xf16>
        %60 = linalg.fill ins(%cst_0 : f16) outs(%34 : tensor<?x?x128xf16>) -> tensor<?x?x128xf16>
      ...
      %44 = loom.subview %arg3[%35, %36, 0] [%0, %1, 128] [1, 1, 1], reuse : [seq = false, spat = false, temp = false] : memref<8x512x128xf16> to memref<?x?x128xf16, strided<[65536, 128, 1], offset: ?>>
      loom.copy_from_tensor %43, %44 : tensor<?x?x128xf16>, memref<?x?x128xf16, strided<[65536, 128, 1], offset: ?>>
