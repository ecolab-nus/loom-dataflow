  ...
  module {
    ...
    func.func @attention(%arg0: memref<2x4096x4096xf32>, %arg1: memref<2x4096x4096xf32>, %arg2: memref<2x4096x4096xf32>, %arg3: memref<2x4096x4096xf32>) {
      ...
      affine.parallel (%arg4, %arg5) = (0, 0) to (2 ceildiv symbol(%0), 4096 ceildiv symbol(%1)) {
        ...
        %4 = tensor.empty(%0, %1) : tensor<?x?x4096xf32>
        %5 = arith.muli %arg4, %0 : index
        %6 = arith.muli %arg5, %1 : index
        %subview = memref.subview %arg2[%5, %6, 0] [%0, %1, 4096] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>
        %7 = bufferization.to_tensor %subview : memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> to tensor<?x?x4096xf32>
        %8 = linalg.fill ins(%cst_1 : f32) outs(%4 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
        ...
        %11:3 = affine.for %arg6 = 0 to affine_map<()[s0] -> (4096 ceildiv s0)>()[%2] iter_args(%arg7 = %10, %arg8 = %9, %arg9 = %8) -> (tensor<?x?xf32>, tensor<?x?xf32>, tensor<?x?x4096xf32>) {
          %14 = arith.muli %arg6, %2 : index
          %subview_5 = memref.subview %arg0[%5, 0, %14] [%0, 4096, %2] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x4096x?xf32, strided<[16777216, 4096, 1], offset: ?>>
          ...
          %18 = linalg.batch_matmul ins(%7, %15 : tensor<?x?x4096xf32>, tensor<?x4096x?xf32>) outs(%17 : tensor<?x?x?xf32>) -> tensor<?x?x?xf32>
          ...
          %subview_6 = memref.subview %arg1[%5, %14, 0] [%0, %2, 4096] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>
          %27 = bufferization.to_tensor %subview_6 : memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>> to tensor<?x?x4096xf32>
          %28 = linalg.fill ins(%cst_1 : f32) outs(%4 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
          %29 = linalg.batch_matmul ins(%22, %27 : tensor<?x?x?xf32>, tensor<?x?x4096xf32>) outs(%28 : tensor<?x?x4096xf32>) -> tensor<?x?x4096xf32>
          %30 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%29, %arg9, %25 : tensor<?x?x4096xf32>, tensor<?x?x4096xf32>, tensor<?x?xf32>) outs(%4 : tensor<?x?x4096xf32>) {
          ^bb0(%in: f32, %in_7: f32, %in_8: f32, %out: f32):
            %31 = arith.mulf %in_7, %in_8 : f32
            %32 = arith.addf %in, %31 : f32
            linalg.yield %32 : f32
          } -> tensor<?x?x4096xf32>
          affine.yield %21, %26, %30 : tensor<?x?xf32>, tensor<?x?xf32>, tensor<?x?x4096xf32>
        }
        %12 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%11#2, %11#1 : tensor<?x?x4096xf32>, tensor<?x?xf32>) outs(%4 : tensor<?x?x4096xf32>) {
        ^bb0(%in: f32, %in_5: f32, %out: f32):
          %14 = arith.divf %in, %in_5 : f32
          linalg.yield %14 : f32
        } -> tensor<?x?x4096xf32>
        %subview_4 = memref.subview %arg3[%5, %6, 0] [%0, %1, 4096] [1, 1, 1] : memref<2x4096x4096xf32> to memref<?x?x4096xf32, strided<[16777216, 4096, 1], offset: ?>>
