  ...
  func.func @flash_decode(%arg0: memref<16x128x8192xf16>, %arg1: memref<16x8192x128xf16>, %arg2: memref<16x32x128xf16>, %arg3: memref<16x32x128xf16>) {
    ...
    affine.parallel (%arg4, %arg5) = (0, 0) to (symbol(%3), symbol(%4)) {
      ...
      %14:3 = scf.for %arg6 = %c0 to %10 step %c1 iter_args(%arg7 = %13, %arg8 = %12, %arg9 = %11) -> (tensor<?x32x1xf16>, tensor<?x32x1xf16>, tensor<?x32x128xf16>) {
        ...
        %30 = loom.bufferize_to_tensor %subview_3[%0, 128, %2] : memref<?x128x?xf16, strided<[1048576, 8192, 1], offset: ?>> -> tensor<?x128x?xf16>
        ...
        %45 = loom.bufferize_to_tensor %subview_4[%0, %2, 128] : memref<?x?x128xf16, strided<[1048576, 128, 1], offset: ?>> -> tensor<?x?x128xf16>
      ...
      %20 = loom.bufferize_to_memref %15 : tensor<?x32x1xf16> -> memref<?x32x1xf16>
      %21 = loom.placeholder [%4, %0, 32, 1] : memref<?x?x32x1xf16>
      loom.gather %20, %21 across(%arg5 : index), area : [1, 1] : memref<?x32x1xf16> to memref<?x?x32x1xf16>
      %22 = loom.bufferize_to_tensor %21[%4, %0, 32, 1] : memref<?x?x32x1xf16> -> tensor<?x?x32x1xf16>
      %23 = tensor.empty(%4, %0) : tensor<?x?x32x128xf16>
      %24 = loom.bufferize_to_memref %18 : tensor<?x32x128xf16> -> memref<?x32x128xf16>
      %25 = loom.placeholder [%4, %0, 32, 128] : memref<?x?x32x128xf16>
      loom.gather %24, %25 across(%arg5 : index), area : [1, 1] : memref<?x32x128xf16> to memref<?x?x32x128xf16>
      %26 = loom.bufferize_to_tensor %25[%4, %0, 32, 128] : memref<?x?x32x128xf16> -> tensor<?x?x32x128xf16>
      %27 = arith.cmpi eq, %arg5, %c0 : index
      scf.if %27 {
        %28 = linalg.fill ins(%cst_1 : f16) outs(%5 : tensor<?x32x1xf16>) -> tensor<?x32x1xf16>
        %29 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%22 : tensor<?x?x32x1xf16>) outs(%28 : tensor<?x32x1xf16>) {
        ^bb0(%in: f16, %out: f16):
          %41 = arith.maximumf %in, %out : f16
          linalg.yield %41 : f16
        } -> tensor<?x32x1xf16>
        %30 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%22, %29 : tensor<?x?x32x1xf16>, tensor<?x32x1xf16>) outs(%19 : tensor<?x?x32x1xf16>) {
        ...
        %32 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%30 : tensor<?x?x32x1xf16>) outs(%31 : tensor<?x32x1xf16>) {
        ^bb0(%in: f16, %out: f16):
          %41 = arith.addf %in, %out : f16
          linalg.yield %41 : f16
        } -> tensor<?x32x1xf16>
        %33 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%30, %32 : tensor<?x?x32x1xf16>, tensor<?x32x1xf16>) outs(%19 : tensor<?x?x32x1xf16>) {
        ^bb0(%in: f16, %in_4: f16, %out: f16):
          %41 = arith.divf %in, %in_4 : f16
          linalg.yield %41 : f16
        } -> tensor<?x?x32x1xf16>
        %34 = tensor.empty(%4, %0) : tensor<?x?x32x32xf16>
        %35 = loom.broadcast ins(%33 : tensor<?x?x32x1xf16>) outs(%34 : tensor<?x?x32x32xf16>) dim(3) -> tensor<?x?x32x128xf16>
        %36 = arith.cmpi eq, %4, %1 : index
        %37 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%26, %35 : tensor<?x?x32x128xf16>, tensor<?x?x32x128xf16>) outs(%23 : tensor<?x?x32x128xf16>) {
        ^bb0(%in: f16, %in_4: f16, %out: f16):
          %41 = arith.mulf %in, %in_4 : f16
          linalg.yield %41 : f16
        } -> tensor<?x?x32x128xf16>
        %38 = linalg.fill ins(%cst : f16) outs(%6 : tensor<?x32x128xf16>) -> tensor<?x32x128xf16>
        %39 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%37 : tensor<?x?x32x128xf16>) outs(%38 : tensor<?x32x128xf16>) {
        ^bb0(%in: f16, %out: f16):
          %41 = arith.addf %in, %out : f16
          linalg.yield %41 : f16
        } -> tensor<?x32x128xf16>
        %subview_3 = memref.subview %arg2[%7, 0, 0] [%0, 32, 128] [1, 1, 1] : memref<16x32x128xf16> to memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
        %40 = loom.bufferize_to_memref %39 : tensor<?x32x128xf16> -> memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
