  ...
  func.func @flash_decode(%arg0: memref<16x128x8192xf16>, %arg1: memref<16x8192x128xf16>, %arg2: memref<16x32x128xf16>, %arg3: memref<16x32x128xf16>) {
    ...
    affine.parallel (%arg4, %arg5) = (0, 0) to (symbol(%3), symbol(%4)) {
      ...
      scf.if %20 {
        ...
        %24 = linalg.fill ins(%cst_0 : f16) outs(%5 : tensor<?x32xf16>) -> tensor<?x32xf16>
        ...
        %26 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%23, %25 : tensor<?x?x32xf16>, tensor<?x32xf16>) outs(%16 : tensor<?x?x32xf16>) {
        ^bb0(%in: f16, %in_5: f16, %out: f16):
          %31 = arith.divf %in, %in_5 : f16
          linalg.yield %31 : f16
        } -> tensor<?x?x32xf16>
        %27 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%19, %26 : tensor<?x?x32x128xf16>, tensor<?x?x32xf16>) outs(%18 : tensor<?x?x32x128xf16>) {
        ^bb0(%in: f16, %in_5: f16, %out: f16):
          %31 = arith.mulf %in, %in_5 : f16
          linalg.yield %31 : f16
        } -> tensor<?x?x32x128xf16>
        %28 = linalg.fill ins(%cst_0 : f16) outs(%6 : tensor<?x32x128xf16>) -> tensor<?x32x128xf16>
        %29 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%27 : tensor<?x?x32x128xf16>) outs(%28 : tensor<?x32x128xf16>) {
        ^bb0(%in: f16, %out: f16):
          %31 = arith.addf %in, %out : f16
          linalg.yield %31 : f16
        } -> tensor<?x32x128xf16>
