...
module {
  module {
    func.func @matmul(%arg0: memref<4096x512xf16>, %arg1: memref<512x4096xf16>, %arg2: memref<4096x4096xf16>) {
      ...
      %cst_0 = arith.constant 0xFC00 : f16 // -inf
      ...
      affine.parallel (%arg3, %arg4) = (0, 0) to (4096 ceildiv symbol(%0), 4096 ceildiv symbol(%1)) {
        ...
        %5 = affine.for %arg5 = 0 to #map()[%2] iter_args(%arg6 = %4) -> (tensor<?x?xf16>) {
          ...
          %15 = tensor.empty(%0) : tensor<?xf16>
          %16 = linalg.fill ins(%cst_0 : f16) outs(%15 : tensor<?xf16>) -> tensor<?xf16>
          %17 = linalg.generic {indexing_maps = [#map1, #map2], iterator_types = ["parallel", "reduction"]} ins(%14 : tensor<?x?xf16>) outs(%16 : tensor<?xf16>) {
          ^bb0(%in: f16, %out: f16):
            %21 = arith.maximumf %in, %out : f16
            linalg.yield %21 : f16
          } -> tensor<?xf16>
          %18 = linalg.generic {indexing_maps = [#map1, #map2, #map1], iterator_types = ["parallel", "parallel"]} ins(%14, %17 : tensor<?x?xf16>, tensor<?xf16>) outs(%3 : tensor<?x?xf16>) {
          ^bb0(%in: f16, %in_2: f16, %out: f16):
            %21 = arith.mulf %in, %in_2 : f16
            linalg.yield %21 : f16
          } -> tensor<?x?xf16>
          %19 = linalg.generic {indexing_maps = [#map1, #map1, #map1], iterator_types = ["parallel", "parallel"]} ins(%arg6, %18 : tensor<?x?xf16>, tensor<?x?xf16>) outs(%3 : tensor<?x?xf16>) {
          ^bb0(%in: f16, %in_2: f16, %out: f16):
            %21 = arith.addf %in, %in_2 : f16
            linalg.yield %21 : f16
          } -> tensor<?x?xf16>
