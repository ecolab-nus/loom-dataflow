module {
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.align %13 by 32
      loom.align %14 by 32
      %15 = loom.intermediate_var : index
      loom.linear_constraint(%15, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%15, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%15, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%15, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %16 = loom.intermediate_var : index
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 32 - 32768)>}
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1048576 + d2 * 1024 - 1073741824)>}
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %15, %16) {map = affine_map<(d0, d1, d2, d3, d4) -> (-d4 + 262144)>}
      %17 = loom.intermediate_var : index
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (-d0 + d1 * 64 + (d2 + d3) * 32 - 2048)>}
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (-d0 + d1 * 2048 + (d2 + d3) * 1024 - 2097152)>}
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (d0 - d1 * 2048 - (d2 + d3) * 32 + 65536)>}
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (d0 - d1 * 64 - (d2 + d3) * 1024 + 65536)>}
      loom.linear_constraint(%12, %14, %13, %17) {map = affine_map<(d0, d1, d2, d3) -> (d3 - 374784)>}
      loom.linear_constraint(%12) {map = affine_map<(d0) -> (d0 - 16)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %14, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %14, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%19, %14, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%19, %14, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %20 = loom.intermediate_var : index
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %21 = loom.intermediate_var : index
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 32 - 32768)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1048576 + d2 * 1024 - 1073741824)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %18, %19, %20, %21) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 64 + d4 * 64 - d6)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%12, %13, %14, %15, %16) {map = affine_map<(d0, d1, d2, d3, d4) -> (-d4 + 262144)>}
      %17 = loom.intermediate_var : index
      loom.linear_constraint(%17, %12, %13, %14) {map = affine_map<(d0, d1, d2, d3) -> (-d0 + (d1 + d2) * 32 + d3 * 64 - 2048)>}
      loom.linear_constraint(%17, %12, %13, %14) {map = affine_map<(d0, d1, d2, d3) -> (-d0 + (d1 + d2) * 1024 + d3 * 2048 - 2097152)>}
      loom.linear_constraint(%17, %12, %13, %14) {map = affine_map<(d0, d1, d2, d3) -> (d0 - (d1 + d2) * 1024 - d3 * 64 + 65536)>}
      loom.linear_constraint(%17, %12, %13, %14) {map = affine_map<(d0, d1, d2, d3) -> (d0 - (d1 + d2) * 32 - d3 * 2048 + 65536)>}
      loom.linear_constraint(%12, %14, %13, %17) {map = affine_map<(d0, d1, d2, d3) -> (d3 - 374784)>}
      loom.linear_constraint(%12) {map = affine_map<(d0) -> (d0 - 16)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      ...
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %20 = loom.intermediate_var : index
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%20, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      ...
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %18, %19, %20, %21) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 960 + d4 * 256 - d6 * 15)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.align %14 by 32
      %15 = loom.intermediate_var : index
      loom.linear_constraint(%15, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%15, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%15, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%15, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %16 = loom.intermediate_var : index
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 32 - 32768)>}
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1048576 + d2 * 1024 - 1073741824)>}
      loom.linear_constraint(%16, %14, %15) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      ...
      loom.linear_constraint(%12) {map = affine_map<(d0) -> (d0 - 16)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %12, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%18, %12, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%18, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%18, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%19, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %20 = loom.intermediate_var : index
      loom.linear_constraint(%20, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%20, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%20, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%20, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %21 = loom.intermediate_var : index
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 32 - 32768)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1048576 + d2 * 1024 - 1073741824)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%12) {map = affine_map<(d0) -> (d0 - 16)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%18, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%18, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%18, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      loom.linear_constraint(%12, %14, %13, %18) {map = affine_map<(d0, d1, d2, d3) -> (d0 * 1024 + d3 - 374784)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %20 = loom.intermediate_var : index
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %21 = loom.intermediate_var : index
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %22 = loom.intermediate_var : index
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 32 - 32768)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1048576 + d2 * 1024 - 1073741824)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %19, %20, %21, %22) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 64 + d4 * 64 - d6)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%16, %15, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1048576 - 1073741824)>}
      loom.linear_constraint(%16, %15, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%16, %15, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1048576 + 33554432)>}
      loom.linear_constraint(%12, %13, %14, %15, %16) {map = affine_map<(d0, d1, d2, d3, d4) -> (-d4 + 262144)>}
      %17 = loom.intermediate_var : index
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (-d0 + d1 * 64 + (d2 + d3) * 32 - 2048)>}
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (-d0 + d1 * 2048 + (d2 + d3) * 1024 - 2097152)>}
      ...
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %19, %20, %21, %22) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 960 + d4 * 256 - d6 * 15)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%18, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%18, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      loom.linear_constraint(%13, %12, %14, %18) {map = affine_map<(d0, d1, d2, d3) -> (d0 * 1024 + d3 - 374784)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%19, %12, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%13, %12, %14, %18) {map = affine_map<(d0, d1, d2, d3) -> (d0 * 1024 + d3 - 374784)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %20 = loom.intermediate_var : index
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%20, %13, %14) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %21 = loom.intermediate_var : index
      loom.linear_constraint(%21, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%21, %13, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%21, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%21, %13, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %22 = loom.intermediate_var : index
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 32 - 32768)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1048576 + d2 * 1024 - 1073741824)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %19, %20, %21, %22) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 256 + d4 * 960 - d6 * 15)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%12) {map = affine_map<(d0) -> (d0 - 16)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %14, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%18, %14, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%18, %14, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%18, %14, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      loom.linear_constraint(%12, %14, %13, %18) {map = affine_map<(d0, d1, d2, d3) -> (d0 * 1024 + d3 - 374784)>}
      %19 = loom.intermediate_var : index
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      ...
      loom.linear_constraint(%19, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %20 = loom.intermediate_var : index
      loom.linear_constraint(%20, %14, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%20, %14, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%20, %14, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
      loom.linear_constraint(%20, %14, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 32 - d2 * 1024 + 32768)>}
      %21 = loom.intermediate_var : index
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%21, %12, %13) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (d0 - d1 * 64 - (d2 + d3) * 1024 + 65536)>}
      loom.linear_constraint(%12, %14, %13, %17) {map = affine_map<(d0, d1, d2, d3) -> (d3 - 374784)>}
      loom.linear_constraint(%12) {map = affine_map<(d0) -> (d0 - 128)>}
      loom.linear_constraint(%13) {map = affine_map<(d0) -> (d0 - 128)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %18, %19, %20, %21) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 256 + d4 * 960 - d6 * 15)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%21, %14, %20) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %18, %19, %20, %21) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 256 + d4 * 256 - d6 * 15)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1048576 - d2 * 32 + 33554432)>}
      loom.linear_constraint(%22, %14, %21) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 1024 + 1048576)>}
      loom.linear_constraint(%12, %13, %14, %19, %20, %21, %22) {map = affine_map<(d0, d1, d2, d3, d4, d5, d6) -> (d3 * 256 + d4 * 256 - d6 * 15)>}
    }
  ...
  module attributes {loom.pass_name = "EnumerateCopyBroadcast"} {
    loom.constraint_space @constraints {
      ...
      loom.linear_constraint(%17, %14, %12, %13) {map = affine_map<(d0, d1, d2, d3) -> (d0 - d1 * 64 - (d2 + d3) * 1024 + 65536)>}
      loom.linear_constraint(%12, %14, %13, %17) {map = affine_map<(d0, d1, d2, d3) -> (d3 - 374784)>}
      loom.linear_constraint(%13) {map = affine_map<(d0) -> (d0 - 16)>}
      %18 = loom.intermediate_var : index
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 32 + d2 * 32 - 1024)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (-d0 + d1 * 1024 + d2 * 1024 - 1048576)>}
      loom.linear_constraint(%18, %14, %12) {map = affine_map<(d0, d1, d2) -> (d0 - d1 * 1024 - d2 * 32 + 32768)>}
