module attributes {loom.tile_b = {is_reduction = false, upper_bound = 8 : index}, loom.tile_k = {is_reduction = false, upper_bound = 512 : index}, loom.tile_m = {is_reduction = false, upper_bound = 4096 : index}, loom.tile_n = {is_reduction = false, upper_bound = 4096 : index}} {
  func.func @batch_matmul(%arg0: memref<8x4096x512xf16>, %arg1: memref<8x512x4096xf16>, %arg2: memref<8x4096x4096xf16>) {
    ...
    affine.parallel (%arg3, %arg4, %arg5) = (0, 0, 0) to (8 ceildiv symbol(%0), 4096 ceildiv symbol(%1), 4096 ceildiv symbol(%2)) {
      %4 = loom.alloc [%0, %3, %2] on @L1 : memref<?x?x?xf16>
      %5 = loom.semaphore_take %4 : memref<?x?x?xf16> -> memref<?x?x?xf16>
      %6 = loom.alloc [%0, %1, %3] on @L1 : memref<?x?x?xf16>
      %7 = loom.semaphore_take %6 : memref<?x?x?xf16> -> memref<?x?x?xf16>
      %8 = loom.alloc [%0, %1, %2] on @L1 : memref<?x?x?xf16>
      %9 = loom.semaphore_take %8 : memref<?x?x?xf16> -> memref<?x?x?xf16>
      %10 = loom.init_tensor %9[%0, %1, %2] : memref<?x?x?xf16> -> tensor<?x?x?xf16>
      %11 = linalg.fill ins(%cst : f16) outs(%10 : tensor<?x?x?xf16>) -> tensor<?x?x?xf16>
      %12 = affine.for %arg6 = 0 to affine_map<()[s0] -> (512 ceildiv s0)>()[%3] iter_args(%arg7 = %11) -> (tensor<?x?x?xf16>) {
        ...
        loom.copy %21, %7 src_mem_space @mem_DRAM dst_mem_space @mem_L1, broadcast : [1, 1] : memref<?x?x?xf16, strided<[2097152, 512, 1], offset: ?>> to memref<?x?x?xf16>
        %22 = loom.bufferize_to_tensor %7[%0, %1, %3] : memref<?x?x?xf16> -> tensor<?x?x?xf16>
        ...
        loom.copy %24, %5 src_mem_space @mem_DRAM dst_mem_space @mem_L1, broadcast : [1, 1] : memref<?x?x?xf16, strided<[2097152, 4096, 1], offset: ?>> to memref<?x?x?xf16>
        %25 = loom.bufferize_to_tensor %5[%0, %3, %2] : memref<?x?x?xf16> -> tensor<?x?x?xf16>
        ...
        loom.semaphore_give %7 : memref<?x?x?xf16>
