        ...
        affine.parallel (%arg5) = (0) to (8) {
          ...
          scf.for %arg6 = %c0 to %25 step %c1 {
            ...
            scf.for %arg7 = %c0 to %26 step %c1 {
              ...
              %49 = linalg.fill ins(%cst_1 : f16) outs(%48 : tensor<?x32xf16>) -> tensor<?x32xf16>
              %50 = loom.alloc [%20, 32] on @L1 : memref<?x32xf16>
              %51 = loom.semaphore_take %50 : memref<?x32xf16> -> memref<?x32xf16>
              %52 = loom.init_tensor %51[%20, 32] : memref<?x32xf16> -> tensor<?x32xf16>
              %53 = linalg.fill ins(%cst_2 : f16) outs(%52 : tensor<?x32xf16>) -> tensor<?x32xf16>
              ...
              %57 = loom.alloc [%20, 32] on @L1 : memref<?x32xf16>
              %58 = loom.semaphore_take %57 : memref<?x32xf16> -> memref<?x32xf16>
              ...
              %71 = loom.alloc [%20, %22, 128] on @L1 : memref<?x?x128xf16>
              %72 = loom.semaphore_take %71 : memref<?x?x128xf16> -> memref<?x?x128xf16>
              %73:3 = scf.for %arg8 = %c0 to %37 step %c1 iter_args(%arg9 = %53, %arg10 = %49, %arg11 = %41) -> (tensor<?x32xf16>, tensor<?x32xf16>, tensor<?x32x128xf16>) {
                ...
                loom.copy %86, %67 src_mem_space @mem_DRAM dst_mem_space @mem_L1, broadcast : [1, 1] region : (UL : [%arg5, %arg4], LR : [%arg5, %arg4]) : memref<?x128x?xf16, strided<[1048576, 8192, 1], offset: ?>> to memref<?x128x?xf16>
                %87 = loom.bufferize_to_tensor %67[%20, 128, %22] : memref<?x128x?xf16> -> tensor<?x128x?xf16>
                %88 = linalg.fill ins(%cst_0 : f16) outs(%70 : tensor<?x32x?xf16>) -> tensor<?x32x?xf16>
                ...
                %90 = linalg.fill ins(%cst_2 : f16) outs(%59 : tensor<?x32xf16>) -> tensor<?x32xf16>
                ...
                %92 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%arg9, %91 : tensor<?x32xf16>, tensor<?x32xf16>) outs(%59 : tensor<?x32xf16>) {
                ...
                %93 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%89, %92 : tensor<?x32x?xf16>, tensor<?x32xf16>) outs(%70 : tensor<?x32x?xf16>) {
                ^bb0(%in: f16, %in_4: f16, %out: f16):
                ...
                %94 = linalg.fill ins(%cst_0 : f16) outs(%62 : tensor<?x32xf16>) -> tensor<?x32xf16>
                ...
                %96 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%arg9, %92 : tensor<?x32xf16>, tensor<?x32xf16>) outs(%65 : tensor<?x32xf16>) {
                ^bb0(%in: f16, %in_4: f16, %out: f16):
                ...
                loom.copy %98, %72 src_mem_space @mem_DRAM dst_mem_space @mem_L1, broadcast : [1, 1] region : (UL : [%arg5, %arg4], LR : [%arg5, %arg4]) : memref<?x?x128xf16, strided<[1048576, 128, 1], offset: ?>> to memref<?x?x128xf16>
                %99 = loom.bufferize_to_tensor %72[%20, %22, 128] : memref<?x?x128xf16> -> tensor<?x?x128xf16>
                %100 = linalg.fill ins(%cst_0 : f16) outs(%56 : tensor<?x32x128xf16>) -> tensor<?x32x128xf16>
              ...
              %74 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%73#1, %73#0 : tensor<?x32xf16>, tensor<?x32xf16>) outs(%46 : tensor<?x32xf16>) {
              ^bb0(%in: f16, %in_4: f16, %out: f16):
                %84 = math.log2 %in : f16
              ...
              loom.semaphore_give %51 : memref<?x32xf16>
              %75 = loom.alloc [%24, %20, 32] on @L1 : memref<?x?x32xf16>
              %76 = loom.semaphore_take %75 : memref<?x?x32xf16> -> memref<?x?x32xf16>
              %77 = loom.init_tensor %76[%24, %20, 32] : memref<?x?x32xf16> -> tensor<?x?x32xf16>
              %78 = loom.gather ins(%74 : tensor<?x32xf16>) outs(%77 : tensor<?x?x32xf16>) across(%arg5 : index) region : (UL : [%c0, %arg4], LR : [%c7, %arg4]) -> tensor<?x?x32xf16>
              loom.semaphore_give %45 : memref<?x32xf16>
              %79 = loom.alloc [%24, %20, 32, 128] on @L1 : memref<?x?x32x128xf16>
              %80 = loom.semaphore_take %79 : memref<?x?x32x128xf16> -> memref<?x?x32x128xf16>
              %81 = loom.init_tensor %80[%24, %20, 32, 128] : memref<?x?x32x128xf16> -> tensor<?x?x32x128xf16>
              %82 = loom.gather ins(%73#2 : tensor<?x32x128xf16>) outs(%81 : tensor<?x?x32x128xf16>) across(%arg5 : index) region : (UL : [%c0, %arg4], LR : [%c7, %arg4]) -> tensor<?x?x32x128xf16>
              loom.semaphore_give %39 : memref<?x32x128xf16>
              %83 = arith.cmpi eq, %arg5, %c0 : index
              scf.if %83 {
                %84 = linalg.fill ins(%cst_2 : f16) outs(%44 : tensor<?x32xf16>) -> tensor<?x32xf16>
                %85 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>], iterator_types = ["reduction", "parallel", "parallel"]} ins(%78 : tensor<?x?x32xf16>) outs(%84 : tensor<?x32xf16>) {
                ^bb0(%in: f16, %out: f16):
                  %92 = arith.maximumf %in, %out : f16
                  linalg.yield %92 : f16
                } -> tensor<?x32xf16>
                %86 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%78, %85 : tensor<?x?x32xf16>, tensor<?x32xf16>) outs(%77 : tensor<?x?x32xf16>) {
                ^bb0(%in: f16, %in_4: f16, %out: f16):
                ...
                %87 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%82, %86 : tensor<?x?x32x128xf16>, tensor<?x?x32xf16>) outs(%81 : tensor<?x?x32x128xf16>) {
                ^bb0(%in: f16, %in_4: f16, %out: f16):
                  %92 = arith.mulf %in, %in_4 : f16
                  linalg.yield %92 : f16
                } -> tensor<?x?x32x128xf16>
                loom.semaphore_give %76 : memref<?x?x32xf16>
                %88 = linalg.fill ins(%cst_0 : f16) outs(%32 : tensor<?x32x128xf16>) -> tensor<?x32x128xf16>
                %89 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d2, d3)>], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%87 : tensor<?x?x32x128xf16>) outs(%88 : tensor<?x32x128xf16>) {
                ^bb0(%in: f16, %out: f16):
                  %92 = arith.addf %in, %out : f16
                  linalg.yield %92 : f16
                } -> tensor<?x32x128xf16>
                loom.semaphore_give %80 : memref<?x?x32x128xf16>
                %90 = loom.subview %arg2[%29, 0, 0] [%20, 32, 128] [1, 1, 1], reuse : [seq = false, spat = true, temp = true] : memref<16x32x128xf16> to memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
                %91 = loom.bufferize_to_memref %89 : tensor<?x32x128xf16> -> memref<?x32x128xf16>
                loom.copy %91, %90 src_mem_space @mem_L1 dst_mem_space @mem_DRAM, broadcast : [8, 1] region : (UL : [%c0, %arg4], LR : [%c7, %arg4]) : memref<?x32x128xf16> to memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
