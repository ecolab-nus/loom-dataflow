    ...
    affine.parallel (%arg4, %arg5) = (0, 0) to (symbol(%3), symbol(%4)) {
      ...
      scf.if %23 {
        ...
        ^bb0(%in: f16, %out: f16, %out_5: i64):
          ...
          %36 = arith.cmpf ogt, %in, %out : f16
          %37 = arith.select %36, %34, %out_5 : i64
          linalg.yield %35, %37 : f16, i64
        } -> (tensor<?x32xf16>, tensor<?x32xi64>)
        ...
        ^bb0(%in: f16, %out: f16):
          %33 = math.powf %cst, %in : f16
          linalg.yield %33 : f16
        } -> tensor<?x?x32xf16>
        %extracted_slice = tensor.extract_slice %28[0, 0, 0] [%4, %0, 32] [1, 1, 1] : tensor<?x?x32xf16> to tensor<?x?x32xf16>
        %expanded = tensor.expand_shape %extracted_slice [[0], [1], [2, 3]] output_shape [%4, %0, 32, 1] : tensor<?x?x32xf16> into tensor<?x?x32x1xf16>
        %29 = arith.cmpi eq, %4, %4 : index
        cf.assert %29, "mismatched size for broadcast"
        %30 = linalg.generic {indexing_maps = [#map5, #map6, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%22, %expanded : tensor<?x?x32x128xf16>, tensor<?x?x32x1xf16>) outs(%21 : tensor<?x?x32x128xf16>) {
        ^bb0(%in: f16, %in_5: f16, %out: f16):
          %33 = arith.mulf %in, %in_5 : f16
          linalg.yield %33 : f16
        } -> tensor<?x?x32x128xf16>
        %31 = linalg.generic {indexing_maps = [#map5, #map7], iterator_types = ["reduction", "parallel", "parallel", "parallel"]} ins(%30 : tensor<?x?x32x128xf16>) outs(%9 : tensor<?x32x128xf16>) {
        ^bb0(%in: f16, %out: f16):
          %33 = arith.addf %in, %out : f16
          linalg.yield %33 : f16
        } -> tensor<?x32x128xf16>
        %subview_4 = memref.subview %arg2[%10, 0, 0] [%0, 32, 128] [1, 1, 1] : memref<16x32x128xf16> to memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
        %32 = bufferization.to_buffer %31 : tensor<?x32x128xf16> to memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
        memref.copy %32, %subview_4 : memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>> to memref<?x32x128xf16, strided<[4096, 128, 1], offset: ?>>
      }
    }
    return
  }
