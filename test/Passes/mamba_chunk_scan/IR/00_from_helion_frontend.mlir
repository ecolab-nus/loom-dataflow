...
module attributes {loom.tile_b = {is_reduction = false, upper_bound = 2 : index}, loom.tile_c = {is_reduction = false, upper_bound = 8 : index}, loom.tile_h = {is_reduction = false, upper_bound = 64 : index}, loom.tile_k = {is_reduction = false, upper_bound = 8192 : index}, loom.tile_m = {is_reduction = false, upper_bound = 256 : index}, loom.tile_n = {is_reduction = false, upper_bound = 64 : index}} {
  func.func @helion_mamba2_chunk_scan_kernel(%arg0: memref<2x8x1x256x256xf16>, %arg1: memref<2x64x8x256xf16>, %arg2: memref<2x64x8x256xf16>, %arg3: memref<2x2048x64x64xf16>, %arg4: memref<2x2048x1x64xf16>, %arg5: memref<2x8x64x64x64xf16>, %arg6: memref<64xf16>, %arg7: memref<2x2048x64x64xf16>) {
    ...
    affine.parallel (%arg8, %arg9, %arg10, %arg11, %arg12) = (0, 0, 0, 0, 0) to (symbol(%6), symbol(%7), symbol(%8), symbol(%9), symbol(%10)) {
      ...
      %expanded = tensor.expand_shape %20 [[0, 1]] output_shape [%0, 1] : tensor<?xf16> into tensor<?x1xf16>
      ...
      %32 = scf.for %arg13 = %c0 to %31 step %c1 iter_args(%arg14 = %28) -> (tensor<?x?xf16>) {
        ...
        %expanded_9 = tensor.expand_shape %17 [[0, 1]] output_shape [%0, 1] : tensor<?xf16> into tensor<?x1xf16>
        %41 = tensor.empty(%0) : tensor<?x1xf16>
        %42 = linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel"]} ins(%expanded_9 : tensor<?x1xf16>) outs(%41 : tensor<?x1xf16>) {
        ^bb0(%in: f16, %out: f16):
          %54 = arith.mulf %in, %cst_1 : f16
          linalg.yield %54 : f16
        } -> tensor<?x1xf16>
        %expanded_10 = tensor.expand_shape %40 [[0, 1]] output_shape [1, %2] : tensor<?xf16> into tensor<1x?xf16>
        %43 = tensor.empty(%2) : tensor<1x?xf16>
        %44 = linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel"]} ins(%expanded_10 : tensor<1x?xf16>) outs(%43 : tensor<1x?xf16>) {
        ^bb0(%in: f16, %out: f16):
          %54 = arith.mulf %in, %cst_1 : f16
          linalg.yield %54 : f16
        } -> tensor<1x?xf16>
        ...
        ^bb0(%in: f16, %in_14: f16, %out: f16):
          %54 = arith.mulf %in, %in_14 : f16
          linalg.yield %54 : f16
        } -> tensor<?x?xf16>
        %subview_11 = memref.subview %arg2[%13, %14, %15, %38] [1, 1, 1, %2] [1, 1, 1, 1] : memref<2x64x8x256xf16> to memref<?xf16, strided<[1], offset: ?>>
        %49 = bufferization.to_tensor %subview_11 : memref<?xf16, strided<[1], offset: ?>> to tensor<?xf16>
        %expanded_12 = tensor.expand_shape %49 [[0, 1]] output_shape [1, %2] : tensor<?xf16> into tensor<1x?xf16>
        %50 = linalg.generic {indexing_maps = [#map1, #map3, #map1], iterator_types = ["parallel", "parallel"]} ins(%48, %expanded_12 : tensor<?x?xf16>, tensor<1x?xf16>) outs(%45 : tensor<?x?xf16>) {
        ^bb0(%in: f16, %in_14: f16, %out: f16):
          %54 = arith.mulf %in, %in_14 : f16
          linalg.yield %54 : f16
        } -> tensor<?x?xf16>
        %subview_13 = memref.subview %arg3[%13, %21, %14, %24] [1, %2, 1, %1] [1, 1, 1, 1] : memref<2x2048x64x64xf16> to memref<?x?xf16, strided<[4096, 1], offset: ?>>
        %51 = bufferization.to_tensor %subview_13 : memref<?x?xf16, strided<[4096, 1], offset: ?>> to tensor<?x?xf16>
        %52 = linalg.matmul ins(%50, %51 : tensor<?x?xf16>, tensor<?x?xf16>) outs(%12 : tensor<?x?xf16>) -> tensor<?x?xf16>
        %53 = linalg.generic {indexing_maps = [#map1, #map1, #map1], iterator_types = ["parallel", "parallel"]} ins(%arg14, %52 : tensor<?x?xf16>, tensor<?x?xf16>) outs(%11 : tensor<?x?xf16>) {
        ^bb0(%in: f16, %in_14: f16, %out: f16):
          %54 = arith.addf %in, %in_14 : f16
          linalg.yield %54 : f16
        } -> tensor<?x?xf16>
        scf.yield %53 : tensor<?x?xf16>
      }
