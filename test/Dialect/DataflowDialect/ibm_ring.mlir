module {
    ...
    %small_rings = df.interconnects "small_rings" %cores, %cores, {map = affine_map<(d0, d1) -> ((d0 + 1) mod 8, d1)>, bandwidth = 32}
    %big_ring = df.interconnects "big_ring" %memories, %memories, {map = affine_map<(d0) -> ((d0 + 1) mod 8)>, bandwidth = 258}
    // Global (DRAM/L2)
    %d = df.spatial_dim "d", 2
    %dram = df.memory "DRAM" {scaleout=(%d) , size = 34359738368, bandwidth = 512}
    %to_dram = df.interconnects %dram, %memories, {map = affine_map<(d0) -> (d0*31)>}
}