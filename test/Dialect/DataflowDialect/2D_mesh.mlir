module {
    ...
    %memories = df.memory "L1", {size = 32768, bandwidth = 64}
    ...
    %drams = df.memory "DRAM", %dram_idx {size = 34359738368, bandwidth = 512}
    %to_dram = df.mux %memories: !df.memory, %drams : !df.memory, {map = affine_map<(d0, d1) -> (d0 ceildiv 4 + 2 * (d1 ceildiv 4))>}
}