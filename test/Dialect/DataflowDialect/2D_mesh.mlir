module {
    ...
    %cores = df.compute "cluster_cores", %x, %y {map = affine_map<(d0, d1) -> (d0, d1)>} : !df.compute
    %memories = df.memory "cluster_mem", %x {map = affine_map<(d0) -> (d0)>} : !df.memory
    %core_to_mem = df.mux %cores: !df.compute, %memories: !df.memory, %x, %y {map = affine_map<(d0, d1) -> (d0)>} : !df.mux
