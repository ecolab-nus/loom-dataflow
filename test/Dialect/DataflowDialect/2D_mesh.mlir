module {
    ...
    %L1 = df.memory "L1" {scaleout=(%x, %y) , size = 1499136, bandwidth = 64}
    %core_to_mem = df.mux %cores, %L1, {map = affine_map<(d0, d1) -> (d0, d1)>}
    %noc_h = df.interconnects "horizontal_links" %L1 : !df.memory, %L1 : !df.memory, {map = affine_map<(d0, d1) -> ((d0 + 1) mod 8, d1)>, bandwidth = 128, spatial_dims = [@y]} : !df.interconnect
    %noc_v = df.interconnects "vertical_links" %L1 : !df.memory, %L1 : !df.memory, {map = affine_map<(d0, d1) -> (d0, (d1 + 1) mod 8)>, bandwidth = 128, spatial_dims = [@x]} : !df.interconnect 
    // dram
