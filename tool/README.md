...
## Loom Opt pipeline
- `affinize` – run the Triton-shared affinization pass only.
  ```bash
  build/tool/loom-opt/single_stage/affinize input.mlir > output.mlir
  ```
- `grid_to_parallel` – wrap kernels in a 3-D `affine.parallel` and drop explicit grid indices.
  ```bash
  build/tool/loom-opt/single_stage/grid_to_parallel input.mlir > output.mlir
  ```
- `enumerate_hw_mapping` – enumerate spatial mappings for a Triton-shared kernel after grid-to-parallel and merge DF declarations provided via `--df`.
  ...
  ```
- `ttshared-opt` – end-to-end pipeline: affinize → grid-to-parallel → spatial exploration. Produces a merged module that contains the DF module and generated clones.
  ```bash
