// Standalone driver to run the Triton-shared spatial reuse analysis on a
// module.
//
// Usage:
//   tmd_triton_shared_reuse <input.mlir>
//
// Prints the module with added `tmd.invariant.{x,y}` attributes on function
// memref arguments.

#include "triton_shared_reuse_analysis.h"
...
int main(int argc, char **argv) {
  ...
  pm.addPass(tmd::passes::createTritonSharedSpatialReuseAnalysisPass());

  if (failed(pm.run(*module))) {
    llvm::WithColor::error(llvm::errs()) << "Analysis pass failed\n";
    return 2;
  }
