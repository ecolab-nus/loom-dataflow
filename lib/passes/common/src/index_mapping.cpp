...
Value emitGlobalIndex2d(OpBuilder &b, Location loc, Value outerIV,
                        Value innerIV, unsigned innerDimSize, Value waveIV,
                        unsigned tileWidth,
                        unsigned totalCores) {
  MLIRContext *ctx = b.getContext();

  // d0=outerIV, d1=innerIV, d2=waveIV
  auto d0 = b.getAffineDimExpr(0);
  auto d1 = b.getAffineDimExpr(1);
  auto d2 = b.getAffineDimExpr(2);

  // 1. Linearize the two logical levels in row-major order.  The stride of
  // the outer level is the extent of the inner level.
  auto linearIdx = d0 * innerDimSize + d1;
  ...
  // innerDimSize=8: linearIdx = [0, 64) offset = [0, 64) globalIdx = waveIV * 64 +
  // offset
  ...
  return affine::AffineApplyOp::create(b, loc, map,
                                       ValueRange{outerIV, innerIV, waveIV});
}
