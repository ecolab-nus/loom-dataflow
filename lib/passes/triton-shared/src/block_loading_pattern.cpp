...
/// @brief Helper to extract static sizes from a range, returning nullopt if any
/// is dynamic.
static std::optional<std::vector<int64_t>>
extractStaticSizes(llvm::ArrayRef<int64_t> sizes) {
 ...
 * @return The block sizes as a vector of constants, or std::nullopt if not all
 * sizes are constants.
 */
std::optional<std::vector<int64_t>>
LoadingBlock::GetBlockSize(mlir::memref::ReinterpretCastOp reinterpret_op) {
  ...
  auto result_type =
      llvm::cast<mlir::MemRefType>(reinterpret_op.getResult().getType());
 ...
 * @return The constant integer value if the value is a constant, std::nullopt
 * otherwise.
 */
...
int64_t LoadingBlock::ExtractDimCoefficientRec(mlir::AffineExpr expr,
                                               int64_t tar_dim_pos) {
 ...
 *
 * @details Extracts the induction variable and upper bound from the current
  ...
  loop_ub_ = std::visit(
      [](auto &op) -> mlir::Value {
        if constexpr (std::is_same_v<std::decay_t<decltype(op)>,
          ...
          // For simplicity, we only handle constant upper bounds for affine.for
          // in this variant for now. Expansion needed for full affine maps.
          return mlir::Value{};
        } else if constexpr (std::is_same_v<std::decay_t<decltype(op)>,
                                            mlir::affine::AffineParallelOp>) {
          return mlir::Value{};
 ...
 *
 * @details Resets the induction variable, upper bound, and constant upper bound
 * to null/empty.
 */
 ...
 */
void LoadingBlock::ResetLoopAttr(
    std::variant<mlir::affine::AffineForOp, mlir::affine::AffineParallelOp>
        new_outer_for) {
  ClearLoopAttr();
  std::visit([&](auto &&op) { outer_for_op_ = op; }, new_outer_for);
  SetLoopAttr();
}
...
LoadingBlock::FindOuterLoopOp() {
  ...
  for (mlir::Operation *parent = current_op->getParentOp(); parent;
    ...
    if (auto affine_parallel =
            mlir::dyn_cast<mlir::affine::AffineParallelOp>(parent)) {
 ...
 * @return The index of the loop induction variable, or std::nullopt if not
 * found.
 */
std::optional<unsigned>
LoadingBlock::GetIvIndex(mlir::OperandRange &original_operands) {
...
/**
 * @brief Create a replacement block for accessing hoisted L1 data at the
 * original location.
 *
 * @details This creates new operations that use the loop induction variable to
 * access the hoisted data.
 */
void LoadingBlock::SetReplacementBlock() {
  // Get original operations
  auto new_reinterpret =
      llvm::dyn_cast<mlir::memref::ReinterpretCastOp>(op_block_[1]);
  auto new_alloc = llvm::dyn_cast<mlir::memref::AllocOp>(op_block_[2]);
  auto original_to_tensor =
      llvm::dyn_cast<mlir::bufferization::ToTensorOp>(org_to_tensor_op_);
  ...
  strides.push_back(
      mlir::IntegerAttr::get(mlir::IndexType::get(builder.getContext()), 1));
  strides.push_back(
      mlir::IntegerAttr::get(mlir::IndexType::get(builder.getContext()), 1));
  strides.push_back(
      mlir::IntegerAttr::get(mlir::IndexType::get(builder.getContext()), 1));
  ...
  auto source_memref_type =
      llvm::cast<mlir::MemRefType>(new_alloc.getResult().getType());
  ...
  auto reduced_result_type =
      mlir::memref::SubViewOp::inferRankReducedResultType(
          reduced_shape, source_memref_type, offsets, sizes, strides);
  ...
  auto new_subview = builder.create<mlir::memref::SubViewOp>(
      ...
      new_alloc.getResult(), offsets, sizes, strides);
  ...
  if (original_to_tensor) {
    ...
    auto new_to_tensor = builder.create<mlir::bufferization::ToTensorOp>(
        original_to_tensor.getLoc(), original_to_tensor.getType(),
        new_subview.getResult(), original_to_tensor.getRestrict(),
        original_to_tensor.getWritable());

    original_to_tensor.getResult().replaceAllUsesWith(
        new_to_tensor.getResult());
...
/**
 * @brief Check if the affine.apply operation is independent of the loop
 * induction variable.
 * @return true if the operation does not depend on loop_iv_, false otherwise.
 */
bool LoadingBlock::IsIndependent() {
  if (auto apply_op =
          llvm::dyn_cast<mlir::affine::AffineApplyOp>(op_block_[0])) {
    mlir::ValueRange operands = apply_op->getOperands();
    if (llvm::is_contained(operands, loop_iv_))
      return false;
    else
      return true;
  }
...
/**
 * @brief Replace the original operations with new hoisted operations and erase
 * the old ones.
...
void LoadingBlock::ReplaceOpBlock(
    ...
    mlir::memref::AllocOp new_alloc, mlir::memref::CopyOp new_copy) {

  auto org_apply = llvm::dyn_cast<mlir::affine::AffineApplyOp>(op_block_[0]);
  auto org_reinterpret =
      llvm::dyn_cast<mlir::memref::ReinterpretCastOp>(op_block_[1]);
...
/**
 * @brief Create a hoisted affine.apply operation by removing specified
 * operands.
 * @param builder The OpBuilder to create operations.
 * @param original_apply The original affine.apply operation.
 * @param index_to_remove The index of the operand to remove, or std::nullopt if
 * none.
 * @return The newly created affine.apply operation.
 */
mlir::affine::AffineApplyOp
LoadingBlock::CreateHoistedApply(mlir::OpBuilder &builder,
  ...
  // Modify affine map: set the dimension to remove as constant 0, remap other
  // dimensions
  ...
  llvm::SmallVector<mlir::AffineExpr> sym_replacements(
      original_map.getNumSymbols());
  ...
  mlir::AffineExpr new_expr =
      original_expr.replaceDimsAndSymbols(dim_replacements, sym_replacements);
  ...
  mlir::AffineMap new_map =
      mlir::AffineMap::get(new_num_dims, original_map.getNumSymbols(),
                           {new_expr}, builder.getContext());

  return builder.create<mlir::affine::AffineApplyOp>(original_apply.getLoc(),
                                                     new_map, new_operands);
}

/**
 * @brief Create hoisted operations with reshaped memory access pattern.
 *
 * @details This method creates new reinterpret_cast and alloc operations with
 * modified sizes and strides to access multiple consecutive memory blocks at
 * once.
 */
void LoadingBlock::CreateHoistedOpsWithReshape(
    mlir::OpBuilder &builder, mlir::affine::AffineApplyOp new_apply,
    mlir::memref::ReinterpretCastOp org_reinterpret,
    mlir::memref::AllocOp org_alloc,
    mlir::memref::ReinterpretCastOp &new_reinterpret,
    mlir::memref::AllocOp &new_alloc) {

  // 2. Extract original static sizes/strides - more reliable to extract from
  // result type
  auto org_result_type =
      llvm::cast<mlir::MemRefType>(org_reinterpret.getResult().getType());
  ...
  if (auto strided_layout =
          llvm::dyn_cast<mlir::StridedLayoutAttr>(org_layout)) {
  ...
  new_static_strides.append(org_static_strides.begin(),
                            org_static_strides.end());

  // 3. Create new static attributes
  auto new_sizes_attr =
      mlir::DenseI64ArrayAttr::get(builder.getContext(), new_static_sizes);
  auto new_strides_attr =
      mlir::DenseI64ArrayAttr::get(builder.getContext(), new_static_strides);
  ...
  auto new_result_type = mlir::MemRefType::get(
      new_result_shape, org_result_type.getElementType(),
  ...
  new_reinterpret = builder.create<mlir::memref::ReinterpretCastOp>(
      org_reinterpret.getLoc(), new_result_type, org_reinterpret.getSource(),
      mlir::ValueRange(new_offsets), mlir::ValueRange(), mlir::ValueRange(),
      mlir::DenseI64ArrayAttr::get(
          builder.getContext(),
          llvm::SmallVector<int64_t>{mlir::ShapedType::kDynamic}),
      new_sizes_attr, new_strides_attr);

  // Create new alloc type: memref<loop_ubx64x64xf32>
  auto org_alloc_type =
      llvm::cast<mlir::MemRefType>(org_alloc.getResult().getType());
  ...
  auto new_alloc_type =
      mlir::MemRefType::get(new_alloc_shape, org_alloc_type.getElementType());

  // Create alloc
  new_alloc =
      builder.create<mlir::memref::AllocOp>(org_alloc.getLoc(), new_alloc_type);
  ...
  if (!org_alloc->getAttrs().empty()) {
    ...
    mlir::NamedAttrList new_attrs;
    new_attrs.append("loom.alloc", mlir::DictionaryAttr::get(
                                       builder.getContext(), alloc_dict_attrs));
    new_alloc->setAttrs(
        mlir::DictionaryAttr::get(builder.getContext(), new_attrs));
  }
}
...
void LoadingBlock::CreateHoistedOpsSimple(
  ...
  llvm::SmallVector<mlir::Value> new_offsets = {new_apply.getResult()};
  new_reinterpret = builder.create<mlir::memref::ReinterpretCastOp>(
      org_reinterpret.getLoc(), org_reinterpret.getResult().getType(),
      org_reinterpret.getSource(), mlir::ValueRange(new_offsets),
      org_reinterpret.getSizes(), org_reinterpret.getStrides(),
 ...
 *
 * @details Creates new operations before the outer_for_op_ and replaces the
 * original ones.
 */
void LoadingBlock::HoistLoadingBlock() {
  ...
  // Check if we need to modify memory access pattern (reshape when loop IV is
  // present and block is multi-dimensional)
  ...
  if (need_reshape) {
    coeff_loop_iv_ =
        ExtractDimCoefficientRec(org_affine_expr, loop_iv_index_opt.value());
 ...
 * @param op_block The sequence of operations: [affine.apply, reinterpret_cast,
 * alloc, copy, to_tensor].
 * @param for_op The innermost scf.for loop operation.
 */
LoadingBlock::LoadingBlock(llvm::SmallVector<mlir::Operation *> op_block,
                           mlir::scf::ForOp for_op)
    : outer_for_op_(for_op),
      op_block_({op_block.begin(), op_block.begin() + 4UL}), is_valid_(false) {
  ...
  if (auto reinterpret_op =
          llvm::dyn_cast<mlir::memref::ReinterpretCastOp>(op_block[1])) {
 ...
 *
 * @details This method recursively hoists the block until the termination
 * condition is met.
 */
void LoadingBlock::HoistRec(
    std::variant<mlir::affine::AffineForOp, mlir::affine::AffineParallelOp>
        new_outer_for) {
  // Early return if we can't determine loop bounds and operation is not
  // independent
  if (!loop_ub_as_const_.has_value() && !IsIndependent()) {
    // If it's AffineParallelOp, we definitely can't determine a constant UB
    // easily here, but we check independence to still allow hoisting if
    // possible.
    if (!IsIndependent())
      return;
  }
  ...
  if (!IsIndependent()) {
    if (!loop_ub_as_const_.has_value() ||
        !mem_req_bytes_as_const_.has_value()) {
      return;
    }
    mem_req_bytes_as_const_ =
        mem_req_bytes_as_const_.value() * loop_ub_as_const_.value();
  }
  ...
  auto next_outer_for = FindOuterLoopOp();
  bool isNull = std::visit(
      [](auto &&op) { return op.getOperation() == nullptr; }, next_outer_for);
  if (!isNull) {
 ...
 */
/// @brief Helper function to get next operation of specific type, or nullptr if
/// not found.
template <typename OpType>
static OpType getNextOpOfType(mlir::Block::iterator &it,
...
BuildLoadingBlocks(mlir::scf::ForOp inner_most_for_op,
  ...
  for (auto it = body->begin(); it != body->end(); ++it) {
    ...
    auto current_it = it;
    auto reinterpret_op = getNextOpOfType<mlir::memref::ReinterpretCastOp>(
        current_it, body->end());
    if (!reinterpret_op)
      continue;

    auto alloc_op =
        getNextOpOfType<mlir::memref::AllocOp>(current_it, body->end());
    if (!alloc_op)
      continue;

    auto copy_op =
        getNextOpOfType<mlir::memref::CopyOp>(current_it, body->end());
    if (!copy_op)
      continue;

    auto to_tensor_op = getNextOpOfType<mlir::bufferization::ToTensorOp>(
        current_it, body->end());
    if (!to_tensor_op)
      continue;
...
/**
 * @brief Hoist a single loading block at the specified index for the innermost
 * loop.
...
mlir::LogicalResult HoistSingleBlock(mlir::scf::ForOp inner_most_loop,
  ...
  // Check if the block is valid (has been hoisted using CreateHoistedOpsSimple
  // at least once)
