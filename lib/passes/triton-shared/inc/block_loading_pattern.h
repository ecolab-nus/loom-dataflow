...
class LoadingBlock {
private:
  std::variant<mlir::scf::ForOp, mlir::affine::AffineForOp,
  ...
  bool is_valid_; // Whether this block has been hoisted using
                  // CreateHoistedOpsSimple at least once

private:
  int64_t GetAllocSizeAsConst(mlir::memref::AllocOp alloc_op);
  std::optional<std::vector<int64_t>>
  GetBlockSize(mlir::memref::ReinterpretCastOp reinterpret_op);
  ...
  bool IsIndependent();
  void ReplaceOpBlock(mlir::affine::AffineApplyOp new_apply,
...
public:
  void ResetLoopAttr(
      std::variant<mlir::affine::AffineForOp, mlir::affine::AffineParallelOp>
          new_outer_for);
  LoadingBlock(llvm::SmallVector<mlir::Operation *> op_block,
  ...
  /// @brief Check if this block is valid (has been hoisted using
  /// CreateHoistedOpsSimple at least once).
...
/// @param forOp The innermost for loop operation (can be AffineForOp or
/// scf::ForOp)
