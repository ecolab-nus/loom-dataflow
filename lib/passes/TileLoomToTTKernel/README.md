...
| `src/MemoryOpToTTKernel.cpp` | `loom.semaphore*`, `loom.copy`, NOC read/write, multicast, reinterpret-cast cleanup | changing DRAM/L1 semantics, broadcast logic, semaphore behavior |
| `inc/ComputeOpToTTKernel.h` | compute conversion API and legality predicates | exposing new compute rewrite helpers |
| `src/ComputeOpToTTKernel.cpp` | matmul/fill/generic/copy lowering into TTKernel ops | adding a new compute op or extending supported generic expressions |
