...
def print_breakdown(variant: dict, assignments: dict[str, int]) -> None:
    """Print a hierarchical timing breakdown for the given symbol assignments.

    Substitutes concrete integer values for all symbols, then evaluates and
    prints every scope → stage → queue resolved_time, plus scope totals and
    the final T_total derivation.
    ...
    print(f"Iteration factors:  seq_iter={seq_val},  temp_iter=[{temp_str}]  (product={temp_product})")
    print()
    ...
    for scope_key in ("compute_scope", "memory_scope"):
        ...
        for stage in scope["stages"]:
            ...
            for q_name, queue in stage["queues"].items():
                ...
                if rt is None:
                    print(f"      {q_name:12s}  (no resolved_time)")
                    continue
            ...
            if queue_vals:
                ...
                for q_name, val in queue_vals.items():
                    ...
                    print(f"      {q_name:12s}  {val:>10,} cycles{marker}")
                print(f"      {'(stage max)':12s}  {stage_max:>10,} cycles")
                scope_total += stage_max

        print(f"  {'→ scope total':16s}  {scope_total:>10,} cycles")
        print()
    ...
    print(f"  T_comp  = {t_comp:>10,} cycles")
    print(f"  T_mem   = {t_mem:>10,} cycles")
    print(f"  T_stage = max(T_comp, T_mem) = {t_stage:,} cycles")
    print(f"  T_total = {t_stage:,} × {seq_val} × {temp_product} = {t_stage * seq_val * temp_product:,} cycles")
